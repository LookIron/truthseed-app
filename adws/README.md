...
The `adws/adw_tests/` directory contains comprehensive tests to validate your ADW system configuration and functionality.

### Available Test Suites

#### 1. Health Check

Validates complete ADW system configuration including environment variables, GitHub authentication, Claude Code CLI, and git repository state.
...
- Claude Code CLI availability and authentication
- GitHub CLI authentication
- Optional: E2B sandbox and Cloudflare R2 configuration

**Returns:** JSON report with pass/fail status for each check

#### 2. Agent Model Tests

Tests integration with Anthropic's Claude models (Opus and Sonnet) in parallel execution.
...
- Opus model execution
- Sonnet model execution
...
#### 3. R2 Screenshot Uploads

Tests Cloudflare R2 integration for screenshot uploads during review phase.
...
- R2 uploader initialization
- Single file upload
- Batch upload with error handling
- Public URL accessibility
- Path resolution

**Requirements:** All Cloudflare R2 environment variables must be set

#### 4. Workflow Validation

Tests workflow dependency validation and orchestration logic.
...
- Entry point workflow validation
...
Tests the model mapping system for slash commands (base vs heavy model sets).
...
- Model lookup for all slash commands
- Base vs heavy model mapping
...
Proof of concept for running Claude Code CLI inside E2B sandboxes with stdin redirection.

```bash
uv run adws/adw_tests/sandbox_poc.py
```
...
**Requirements:** `E2B_API_KEY` and `ANTHROPIC_API_KEY` must be set
...
For initial setup validation, run tests in this order:

```bash
# 1. Test workflow validation (no external dependencies)
uv run adws/adw_tests/test_webhook_simplified.py

# 2. Test model selection (no external dependencies)
python adws/adw_tests/test_model_selection.py

# 3. Run health check (requires ANTHROPIC_API_KEY)
uv run adws/adw_tests/health_check.py

# 4. Test agent models (requires ANTHROPIC_API_KEY)
uv run adws/adw_tests/test_agents.py

# 5. Test R2 uploads (requires Cloudflare R2 config)
uv run adws/adw_tests/test_r2_uploader.py

# 6. Test E2B sandbox (requires E2B_API_KEY)
uv run adws/adw_tests/sandbox_poc.py
```
...
All test scripts use inline dependency declarations. The `uv run` command automatically installs required dependencies.
...
Ensure Claude Code is in your PATH or set `CLAUDE_CODE_PATH`:
